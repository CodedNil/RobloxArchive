...
local Ragdolls = workspace:FindFirstChild("Ragdolls") or Instance.new("Model", workspace)
Ragdolls.Name = "Ragdolls"
...
character:WaitForChild("Humanoid").Died:connect(function()
	...
	local Gun = character:FindFirstChild("M1 Garand")
	if Gun then
		...
		Gun.Wood.CanCollide = true
		
		local Weld = Instance.new("Weld", Gun.Wood)
		Weld.Part0 = Gun.Wood
		Weld.Part1 = Gun.Metal
		Weld.C0 = CFrame.new(0.13, 0.21, -1.19)
	end
	...
	for _, v in pairs(character:GetDescendants()) do
		if v:IsA("BasePart") then
			v.Anchored = true
		elseif v:IsA("BallSocketConstraint") or v:IsA("HingeConstraint") or v:IsA("Attachment") or v:IsA("Vector3Value") or v:IsA("BrickColorValue") or v:IsA("Folder") or v:IsA("Animator") or v:IsA("Sound") then
			v:Destroy()
		end
	end
	...
	if Gun and Gun:FindFirstChild("Wood") and Gun:FindFirstChild("Metal") then
		if Gun:FindFirstChild("GunStat") then
			Gun.GunStat:Destroy()
		end
		Gun.Wood.Weld:Destroy()
		Gun.Metal.CFrame = Gun.Wood.CFrame * CFrame.new(0.13, 0.21, -1.19)
		Gun.Wood.Anchored = true
		Gun.Metal.Anchored = true
	end
